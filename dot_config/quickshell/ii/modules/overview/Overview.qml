...
Scope {
    ...
    function toggleGlyphs() {
        ...
        for (let i = 0; i < overviewVariants.instances.length; i++) {
            ...
            if (panelWindow.modelData.name == Hyprland.focusedMonitor.name) {
                overviewScope.dontAutoCancelSearch = true;
                // Assumes you have this prefix configured:
                // Config.options.search.prefix.glyphs → e.g. ":glyph "
                panelWindow.setSearchingText(Config.options.search.prefix.glyphs);
                GlobalStates.overviewOpen = true;
                return;
            }
        }
    ...
    GlobalShortcut {
        name: "overviewGlyphToggle"
        description: "Toggle glyph query on overview widget"
        onPressed: {
            overviewScope.toggleGlyphs();
        }
    }
